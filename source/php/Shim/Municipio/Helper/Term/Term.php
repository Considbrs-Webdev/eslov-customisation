...
use WP_Term;
use WpService\Contracts\{ApplyFilters, GetAncestors, GetTermBy, IsWpError, WpGetAttachmentImageUrl, WpInsertTerm};

/**
 * Patched Term helper — separate icon/colour caches.
 *
 * Upstream Municipio shares one static cache between getTermIcon() and getTermColor()
 * with identical keys, so TermIconResolver fatals when both run for the same term.
 * Remove this preload when the theme ships separate caches upstream.
 */
class Term implements GetTermColor, GetTermIcon, CreateOrGetTermIdFromString
{
    public static array $colorCache = [];

    public static array $iconCache = [];
    ...
    public function getTermColor(int|string|\WP_Term $term, string $taxonomy = ''): false|string
    {
        ...
        if (array_key_exists($cacheKey, self::$colorCache)) {
            return self::$colorCache[$cacheKey];
        }

        if (empty($term)) {
            return self::$colorCache[$cacheKey] = false;
        }
        ...
        return self::$colorCache[$cacheKey] = $result;
    }
    ...
    public function getTermIcon(int|string|\WP_Term $term, string $taxonomy = ''): array|false
    {
        ...
        if (array_key_exists($cacheKey, self::$iconCache)) {
        ...
        if (empty($term)) {
            return self::$iconCache[$cacheKey] = false;
        }
        ...
        return self::$iconCache[$cacheKey] = $result;
    }
